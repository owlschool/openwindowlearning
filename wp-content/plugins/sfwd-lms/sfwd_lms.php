...
function sfwd_lms_has_access( $post_id, $user_id = null, &$reason = null ) {
	...
	if (@$meta['sfwd-courses_course_price_type'] == "closed") {
		if ($reason != null)
		   $reason = 'closed';
    ...
    if (( $post_type == 'sfwd-lessons' )  || ( $post_type == 'sfwd-topic' ) || ( $post_type == 'sfwd-courses' )) {
        ...
        if (strstr($agent, "googlebot") || strstr($agent, "bingbot") || strstr($agent, "yahoo"))
            return true;

        // Check FCF
        if (strstr(strtolower($_SERVER['HTTP_REFERER']), "google.") || strstr(strtolower($_SERVER['HTTP_REFERER']), "bing.")) {
            $thismonth = date("n");
            $count = 1;
            $month = $thismonth;
            if(!empty($_COOKIE['owl_fcf_count']))
            {
                $parts = explode(",", $_COOKIE['owl_fcf_count']);				
                $month = $parts[1];
                if($month == $thismonth)
            ...
            setcookie("owl_fcf_count", $count . "," . $month, time()+3600*24*31, "/");
            return true;			
        }
    }
    ...
    if (pmpro_hasMembershipLevel(5, $user_id) && (( $post_type == 'sfwd-lessons' )  || ( $post_type == 'sfwd-topic' ) || ( $post_type == 'sfwd-courses' )) ) {
        if ($reason != null)
            $reason = 'wrong_membership';
...
function sfwd_lms_access_redirect( $post_id ) {
	$reason = "";
	$access = sfwd_lms_has_access( $post_id, null, $reason );
	if ( $access === true ) 
		return true;
	
	if ($reason == 'closed') {
		$link = '/course-not-supported/';
	} else if ($reason == 'fcf_limit') {
		$link = '/free-articles-limit/';
	} else if ($reason == 'wrong_test') {
		$link = '/course-not-selected/';
	} else if ($reason == 'wrong_membership') {
		$link = '/incorrect-membership-level/';
	} else {
	    $link = '/join/';
	}
		...
		function add_post_types() {
			...
			if(is_admin()) {
			  ...
			  $wppro_categories = $wppro_categorymapper->fetchAll();
                          ...
                          foreach ($wppro_categories as $wppro_category) {
                              $lesson_categories[$wppro_category->getCategoryId()] = $wppro_category->getCategoryName();
                          }
                          $lesson_categories[0] = 'None';

                          $quiz_mapper = new WpProQuiz_Model_QuizMapper();
                          $quizzes = $quiz_mapper->fetchAll();
                          $quiz_list = array();
                          foreach($quizzes as $q) {
                              $quiz_list[$q->getId()] = $q->getId()." - ".$q->getName();
                          }
			...
			$post_args = Array(
					  ...
					  Array( 'course_materials' => 
					   ...
					   'course_price_type' => 
						...
						'course_test_assignment' => Array( 
							...
							'default' => 'ged',
							'help_text' => 'Choose the test assigned for this course.'
							),
					...
					//  'taxonomies' => Array( 'courses' => __('Manage Course Associations', 'learndash') ),
					  ...
					  'options_page_title' => "Assessment Test Settings",
					  'fields' => Array(
						...
						'lesson_category_in_assessment_test' => Array( 
							'name' => 'Category In Assessment Test',
							'type' => 'select',
							'initial_options' => $lesson_categories, 
							'default' => '0',
							'help_text' => 'Choose the category of questions in the assessment test that evaluate this lesson.'
							),
					  ...
					  'default_options' => Array(
						...
						'ged_assessment'		=> Array( 'name' => 'GED Assessment Test', 'help_text' => 'Select which quiz acts as the assessment', 'type' => 'select', 'initial_options' => $quiz_list),
						'tasc_assessment'		=> Array( 'name' => 'TASC Assessment Test', 'help_text' => 'Select which quiz acts as the assessment', 'type' => 'select', 'initial_options' => $quiz_list),
						'hiset_assessment'		=> Array( 'name' => 'HiSET Assessment Test', 'help_text' => 'Select which quiz acts as the assessment', 'type' => 'select', 'initial_options' => $quiz_list),
												),
					),
					Array(
