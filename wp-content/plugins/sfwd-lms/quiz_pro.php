...
function learndash_has_user_taken_assessment($user_id)
{
    if(empty($user_id))
    ...
    $test_choice = get_user_meta($user_id, 'test_choice', true);

    $lesson_options = learndash_get_option("sfwd-lessons");
    if(!empty($lesson_options))
        extract($lesson_options);
			
    $quiz_to_query = $ged_assessment;

    if ($test_choice == 'ged')
        $quiz_to_query = $ged_assessment;

    else if ($test_choice == 'hiset')
        $quiz_to_query = $hiset_assessment;

    else if ($test_choice == 'tasc')
        $quiz_to_query = $tasc_assessment;

    if (empty($quiz_to_query))
        return true;

    $ld_quiz_id = LD_QuizPro::get_ld_quiz_id($quiz_to_query);
    ...
    if ( !empty( $usermeta ) )	{
	    ...
	    if ( $v['quiz'] == $ls_quiz_id ) return true;
        }
    }
