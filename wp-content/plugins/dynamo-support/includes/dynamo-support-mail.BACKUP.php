...
$last_checked = get_transient('support_dynamo_mailserver_last_checked');

//if ( $last_checked )
	//wp_die(__('Slow down cowboy, no need to check for new mails so often!'));

set_transient('support_dynamo_mailserver_last_checked', true, WP_MAIL_INTERVAL);
...
if ( !$pop3->connect( 'mail.plugindynamo.com', '110' ) || !$pop3->user( 'support+plugindynamo.com' ) )
	wp_die( esc_html( $pop3->ERROR ) );

$count = $pop3->pass( 'Lu$$V&RpD!dd' );
...
for ( $i = 1; $i <= $count; $i++ ) {
	...
	foreach ($message as $line) {
		...
		if ( $bodysignal ) {
			...
			if ( ( $content_type == 'multipart/alternative' ) && ( false !== strpos($line, 'boundary="') ) && ( '' == $boundary ) ) {
				...
				$boundary = $boundary[1];
			} else if ( ( $content_type == 'multipart/alternative' ) && ( false !== strpos($line, 'boundary=') ) && ( '' == $boundary ) ) {
				$boundary = trim($line);
				$boundary = explode('=', $boundary);
				$boundary = $boundary[1];
			}
	...
	if ( $content_type == 'multipart/alternative' ) {
		...
		$content = strip_tags($content, '<img><p><br><i><b><u><em><strong><strike><font><span><div><style>');
	}
	...
	$post_data = compact('post_content','post_title','post_date','post_date_gmt','post_author', 'post_status','post_type', 'comment_status','ping_status');
