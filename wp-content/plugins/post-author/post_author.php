...
function add_author_to_post($content)
{
    ...
    if( (is_page() && ($options['post_author_type_page'] == 'on') && in_the_loop() ) || (is_single() && ($options['post_author_type_post'] == 'on') && in_the_loop() ) )
    {
        ...
        if( $author_exception != 'on' )
        {
            ...
            $content .='<span class="post_author_author">' . (ICL_LANGUAGE_CODE === 'es' ? 'Redactado por: ' : $options['post_author_txt_before']) . ' ';
            $content .= $author_link_start;
            $content .= $author;
            $content .= $author_link_end;
            ...
            {
                $content .= '<span class="post_author_create">' . (ICL_LANGUAGE_CODE === 'es' ?  'el ' : $options['post_author_dat_before']) . $post_date . $options['post_author_dat_after'] . '</span>';
            }
...
//    $post_author_modify_link_name = '';
    ...
    if( ((is_home() && !($options['post_author_cat_home'] == 'on')) || is_category()) && ($options['post_author_type_cat'] == 'on') )
    {
        if( $author_exception != 'on' )
        {
            $content = '<div class="post_author_plugin_cat"><span class="post_author_author">' . (ICL_LANGUAGE_CODE === 'es' ? 'Redactado por: ' : $options['post_author_txt_before']) . ' ';
