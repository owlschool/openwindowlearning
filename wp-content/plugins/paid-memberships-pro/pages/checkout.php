...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<tbody>                
		<tr>
			<td>You are subscribing to GED Board's Tutoring Service!	
	...
	<?php } ?>
        ...
        <div style="width:100%; float: none; clear: none; padding-top: 5px;">
        <div style="width:45%; float: left; clear: none;">
	<?php
		do_action('pmpro_checkout_after_pricing_fields');
	?>
	
	<?php if(!$skip_account_fields && !$pmpro_review) { ?>
	<table id="pmpro_user_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0px;">
	<thead>
		<tr>					
			<th>
                                <?php _e('Account Information', 'pmpro');?>
				<span class="pmpro_thead-msg" style="text-align:left;"><?php _e('Already have an account?', 'pmpro');?> <a href="<?php echo wp_login_url(pmpro_url("checkout", "?level=" . $pmpro_level->id))?>"><?php _e('Log in here', 'pmpro');?></a>.</span>
			</th>						
		</tr>
	</thead>
	<tbody>                
	...
	<table id="pmpro_payment_information_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if(!$pmpro_requirebilling || $gateway == "paypalexpress" || $gateway == "paypalstandard" || $gateway == "twocheckout") { ?>style="display: none; margin-top:0px;"<?php } else { ?>style="margin-top:0px;" <?php } ?>>
	<thead>
		<tr>
			<th><?php _e('Payment Information', 'pmpro');?>
				<?php
	...
	<?php					
		...
		<tbody>
			<tr>
				<td>								
					<input type="checkbox" name="tos" value="1" id="tos" /> <label class="pmpro_normal pmpro_clickable" for="tos"><?php printf(__('I agree to the GEDBoard.com %s', 'pmpro'), $tospage->post_title);?></label>
				</td>
			</tr>
		</tbody>
		</table>
